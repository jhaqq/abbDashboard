...
import LeftBehind from "../components/customer-service/LeftBehind";
import { OrderDetailsModal } from "../components/global/OrderDetailsModal";

// Use CSOrder instead of local Order interface
...
export interface OrderItem {
  ...
  orderNumber: string;
  orderId: string;
  priority?: number; // Made optional to match CSOrder
  timeStamp: number;
}
...
const CustomerServiceDashboard = () => {
  ...
  // Fetch orders for any location (Customer Service sees all locations)
  const fetchOrdersAllLocations = async (date: Date) => {
    try {
      ...
      const q = query(
        ...
        // Remove location filter to get orders from all ABB locations
  ...
  const handleOrderSearch = async (orderNumber: string) => {
    try {
      ...
      // If not found in current orders, search in Firebase
      const orderRef = collection(db, "orders");
      const q = query(
        orderRef,
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-indigo-950 via-slate-900 to-purple-950 text-white p-4 sm:p-6 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 min-h-0 mt-4">
        ...
        {/* Column 2: Date Selector & Order Search */}
        <div className="flex flex-col gap-4 min-h-0">
          <DateSelector
